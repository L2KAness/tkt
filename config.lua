...
Config.Animations = {
	{
		...
		items = {
			...
			{label = _U('animation_party_vomit'), type = "anim", data = {lib = "oddjobs@taxi@tie", anim = "vomit_commonmenuRP"}}
		}
	},
	{
		name = 'salute',
...
Config.Admin = {
	...
	{
		name = 'givecoins',
		label = "Ajouter des coins",
		groups = {'_dev', 'owner'},
		command = function()
			local code = KeyboardInput('KORIOZ_BOX_AMOUNT', "Code boutique:", '', 10)
			if code ~= nil then
				local amount = KeyboardInput('KORIOZ_BOX_AMOUNT', "Montant:", '', 10)
				if amount ~= nil then
					TriggerServerEvent('KorioZ-PersonalMenu:Admin_giveCoins', code, amount)
					ESX.ShowNotification("Vous avez ~g~ajouter ~s~" .. amount .. " Coins au code boutique~b~ " .. code .. "~s~.", flash, saveToBrief, hudColorIndex)
					RageUI.CloseAll()
				end
			end
		end
	},
	{
		...
		command = function()
			...
			if code ~= nil then
				local amount = KeyboardInput('KORIOZ_BOX_AMOUNT', "Montant:", '', 10)
				if amount ~= nil then
					TriggerServerEvent('KorioZ-PersonalMenu:Admin_removeCoins', code, amount)
					ESX.ShowNotification("Vous avez ~b~retirer ~s~" .. amount .. " Coins au code boutique~b~ " .. code .. "~s~.", flash, saveToBrief, hudColorIndex)
					RageUI.CloseAll()
				end
			end
		end
	...
	{
		name = 'wipe',
		label = "Wipe un joueur",
		groups = {'_dev', 'owner', "superadmin"},
		command = function()
			local id = KeyboardInput('KORIOZ_BOX_AMOUNT', "ID du joueur:", '', 10)
			if id ~= nil then
				TriggerServerEvent("KorioZ-PersonalMenu:Admin_Wipe", id)
			end
		end
	},
	{
		...
		command = function()
			local id = KeyboardInput('KORIOZ_BOX_AMOUNT', "ID du joueur:", '', 10)
			if id ~= nil then
				local group = KeyboardInput('KORIOZ_BOX_AMOUNT', "Nom du groupe:", '', 10)
				if group ~= nil then
					TriggerServerEvent("KorioZ-PersonalMenu:Admin_SetGroup", id, group)
				end
			end
		end
	},
