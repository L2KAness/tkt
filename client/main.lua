...
Citizen.CreateThread(function()
	...
	RMenu.Add('personal', 'boutique', RageUI.CreateSubMenu(RMenu.Get('tebex', 'main')))
...
RegisterNetEvent("closeAll:PersonalMenu")
AddEventHandler("closeAll:PersonalMenu", function()
	RageUI.CloseAll()
end)
...
function RenderPersonalMenu()
	RageUI.DrawContent({header = true, instructionalButton = true}, function()
		...
		RageUI.Button("             Connecté en tant qu'id [~b~"..GetPlayerServerId(PlayerId()).."~s~]", nil, {}, true, function(Hovered, Active, Selected)
			if (Selected) then
			end
		end, RMenu.Get('', ''))
		
		RageUI.Button("~b~→ Boutique", nil, {}, true, function(Hovered, Active, Selected)
			if (Selected) then
				RageUI.CloseAll()
				exports['Kalops']:MenuBoutique()
			end
		end)

		RageUI.Info('~b~Sea~s~Life', {'~b~Votre ~s~ID : ', '', '~b~Jobs : ','Grade ~s~Job : ', '', '~b~Gangs : ', 'Grade ~s~Gang : '}, { '~b~'..GetPlayerServerId(PlayerId()), '', ESX.PlayerData.job.label, ESX.PlayerData.job.grade_label, '', ESX.PlayerData.job2.label, ESX.PlayerData.job2.grade_label})
	
		RageUI.Button("> Inventaire", "Accéder à votre inventaire", {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
			if (Selected) then
			end
		end, RMenu.Get('personal', 'inventory'))

		RageUI.Button("> Portefeuille", "Votre Portefeuille", {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
			if (Selected) then
			end
		end, RMenu.Get('personal', 'wallet'))


		RageUI.Button("> Armes", "Vos armes", {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
			if (Selected) then
			end
		end, RMenu.Get('personal', 'loadout'))


		RageUI.Button("> Factures", "Vos factures", {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
			if (Selected) then
			end
		end, RMenu.Get('personal', 'billing'))

		RageUI.Button("> Vêtements", "Actions sur vos vêtements", {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
			if (Selected) then
			end
		end, RMenu.Get('personal', 'clothes'))
		...
		if IsPedSittingInAnyVehicle(plyPed) then
			if (GetPedInVehicleSeat(GetVehiclePedIsIn(plyPed, false), -1) == plyPed) then
				...
				end, RMenu.Get('personal', 'vehicle'))
			end
		end

		if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.grade_name == 'boss' then
			RageUI.Button("> Gestion Entreprise", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
				if (Selected) then
				end
			end, RMenu.Get('personal', 'boss'))
		end

		if ESX.PlayerData.job2 ~= nil and ESX.PlayerData.job2.grade_name == 'boss' then
			RageUI.Button("> Gestion Organisation", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
				if (Selected) then
				end
			end, RMenu.Get('personal', 'boss2'))
		end

		if ESX.PlayerData.group == "superadmin" or ESX.PlayerData.group == "_dev"then
			RageUI.Button("~b~Administration", nil, {}, true, function(Hovered, Active, Selected)
				if (Selected) then
				end
			end, RMenu.Get('personal', 'admin'))
		end
	end)
end
...
AddEventHandler("openTebex", function()
    getPlayerInfos()
    RageUI.Visible(RMenu:Get('tebex', 'main'), not RageUI.Visible(RMenu:Get('tebex', 'main')))
end)
